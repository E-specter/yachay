...
@Component({
  selector: 'app-student-notificaciones',
  imports: [AppIcon],
  templateUrl: './notificaciones.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class StudentNotificaciones implements OnInit {
  ...
  markAsRead(notification: UserNotification): void {
    ...
    this.notificationService.markAsRead('ALUMNO', notification.id).subscribe({
      ...
      error: (error) => {
        console.error('Error marcando notificacion alumno', error);
  ...
  markAllAsRead(): void {
    this.notificationService.markAllAsRead('ALUMNO').subscribe({
      ...
      error: (error) => {
        console.error('Error marcando todas las notificaciones alumno', error);
